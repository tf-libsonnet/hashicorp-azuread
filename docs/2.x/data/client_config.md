...
`client_config` represents the `azuread_client_config` Terraform data source.
...
`azuread.client_config.withTimeouts` constructs a mixin object that can be merged into the `client_config`
...
`azuread.client_config.withTimeoutsMixin` constructs a mixin object that can be merged into the `client_config`
Terraform data source block to set or update the timeouts field.
